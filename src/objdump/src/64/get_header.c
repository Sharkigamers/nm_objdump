...
void print_64_flags(unsigned short flags)
{
    const static map64_t needed_flags[] = {{NO_FLAGS, "NO_FLAGS"},
...
char *get_64_architecture(int architecture)
{
    static const map64_t machines[] = {{EM_NONE, "None"}, {EM_M32, "WE32100"},
...
unsigned short get_64_flags(Elf64_Ehdr *elf)
{
...
{
    unsigned short flags = get_64_flags(elf);

    printf("architecture: %s, flags 0x%08x:\n",
    get_64_architecture(elf->e_machine), flags);
    print_64_flags(flags);
