...
int my_special_64_strcmp(char const *s1, char const *s2, char flag1, char flag2)
{
    int i = 0;
    int j = 0;
...
int compare_64_names(const char *s1, const char *s2, char flag1, char flag2)
{
    ...
    return_value = my_special_64_strcmp(dumped_s1, dumped_s2, flag1, flag2);
    free(dumped_s1);
    free(dumped_s2);
    return return_value;
}

void swap_linked_64_values(symbol64_t *current_list)
{
...
void sort_64_struct(symbol64_t *symbol_list)
{
    ...
    while (sorted < 2) {
        ...
        current_list = current_list->next) {
            if (compare_64_names(current_list->name, current_list->next->name,
            current_list->flag, current_list->next->flag) > 0) {
                swap_linked_64_values(current_list);
                sorted = 0;
            }
        }
    }
